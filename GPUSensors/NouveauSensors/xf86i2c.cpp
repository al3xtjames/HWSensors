...
I2CRaiseSCL(I2CBusPtr b, int sda, int timeout)
{
    ...
    if (i <= 0) {
        I2C_TIMEOUT(IOLog("[I2CRaiseSCL(<%s>, %d, %d) timeout]\n", b->BusName, sda, timeout));
        return FALSE;
    }
...
I2CPutByte(I2CDevPtr d, I2CByte data)
{
    ...
    if (r) {
        ...
        if (i <= 0) {
            I2C_TIMEOUT(IOLog("[I2CPutByte(<%s>, 0x%02x, %d, %d, %d) timeout]",
                              b->BusName, data, d->BitTimeout,
                              d->ByteTimeout, d->AcknTimeout));
            r = FALSE;
        }
        
        I2C_TRACE(IOLog("W%02x%c ", (int) data, sda ? '-' : '+'));
    }
...
I2CGetByte(I2CDevPtr d, I2CByte *data, Bool last)
{
    ...
    *data = (sda > 0) << 7;
    ...
    I2C_TRACE(IOLog("R%02x%c ", (int) *data, last ? '+' : '-'));
...
xf86I2CDevInit(I2CDevPtr d)
{
    ...
    if (d == NULL || (b = d->pI2CBus) == NULL || (d->SlaveAddr & 1) || xf86I2CFindDev(b, d->SlaveAddr) != NULL) {
        I2C_TRACE("xf86I2CDevInit = FALSE");
        return FALSE;
    }
...
xf86I2CFindDev(I2CBusPtr b, I2CSlaveAddr addr)
{
    ...
    if (b) {
        for (d = b->FirstDev; d != NULL; d = d->NextDev)
            if (d->SlaveAddr == addr) {
                I2C_TRACE("xf86I2CFindDev = %s", d ? "TRUE" : "FALSE");
                return d;
            }
    }
